...
typedef struct {
    ...
    XMobilenetv2_block_uio_map maps[ MAX_UIO_MAPS ];
} XMobilenetv2_block_uio_info;

/***************** Variable Definitions **************************************/
static XMobilenetv2_block_uio_info uio_info;
...
static int uio_info_read_name(XMobilenetv2_block_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMobilenetv2_block_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMobilenetv2_block_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMobilenetv2_block_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMobilenetv2_block_Initialize(XMobilenetv2_block *InstancePtr, const char* InstanceName) {
	XMobilenetv2_block_uio_info *InfoPtr = &uio_info;
