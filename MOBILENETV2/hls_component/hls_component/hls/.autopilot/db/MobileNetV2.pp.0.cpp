...
void depthwise_conv(
    ...
    const data_t dw_weights[32][3][3]
) {

    data_t line_buffer[3][56];
...
#pragma HLS ARRAY_PARTITION variable=window complete dim=0
 ...
 VITIS_LOOP_27_1: for (int ch = 0; ch < 32; ch++) {
        VITIS_LOOP_28_2: for (int r = 0; r < 56; r++) {
            VITIS_LOOP_29_3: for (int c = 0; c < 56; c++) {
#pragma HLS PIPELINE II=1


 data_t new_pixel = in_stream.read();
                VITIS_LOOP_34_4: for (int i = 0; i < 3 - 1; i++) {
                    line_buffer[i][c] = line_buffer[i+1][c];
                }
                line_buffer[3 -1][c] = new_pixel;


                VITIS_LOOP_40_5: for (int i = 0; i < 3; i++) {
                    VITIS_LOOP_41_6: for (int j = 0; j < 3 - 1; j++) {
                        window[i][j] = window[i][j+1];
                    }
                    window[i][3 -1] = line_buffer[i][c];
                }


                if (r >= 2 && c >= 2) {
                    data_t sum = 0;
                    VITIS_LOOP_50_7: for (int kr = 0; kr < 3; kr++) {
                        VITIS_LOOP_51_8: for (int kc = 0; kc < 3; kc++) {
...
void pointwise_conv(
    ...
    const data_t pw_weights[32][32]
) {


    VITIS_LOOP_72_1: for (int r = 0; r < 56 - 2; r++) {
        VITIS_LOOP_73_2: for (int c = 0; c < 56 - 2; c++) {


            data_t pixel_channels[32];
            VITIS_LOOP_77_3: for (int ch = 0; ch < 32; ch++) {
#pragma HLS PIPELINE II=1
 pixel_channels[ch] = in_stream.read();
            }


            VITIS_LOOP_83_4: for (int oc = 0; oc < 32; oc++) {
#pragma HLS PIPELINE II=1
 data_t sum = 0;
                VITIS_LOOP_86_5: for (int ic = 0; ic < 32; ic++) {
                    sum += pixel_channels[ic] * pw_weights[oc][ic];
                }
                output_tensor[(oc * (56 -2) * (56 -2)) + (r * (56 -2)) + c] = sum;
            }
        }
    }
}
...
__attribute__((sdx_kernel("mobilenetv2_block", 0))) void mobilenetv2_block(
    data_t *input_tensor,
    data_t *output_tensor,
    const data_t dw_weights[32][3][3],
    const data_t pw_weights[32][32]
...
#line 1 "directive"
#pragma HLSDIRECTIVE TOP name=mobilenetv2_block
# 103 "MobileNetV2.cpp"
...
#pragma HLS STREAM variable=internal_fifo depth=64
 ...
 hls::stream<data_t> input_stream("in_stream");
    VITIS_LOOP_117_1: for(int i=0; i<32*56*56; i++) input_stream.write(input_tensor[i]);
