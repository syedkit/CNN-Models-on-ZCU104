...
__attribute__((sdx_kernel("resnet50_bottleneck", 0))) void resnet50_bottleneck(
    data_t *input_tensor,
...
#line 1 "directive"
#pragma HLSDIRECTIVE TOP name=resnet50_bottleneck
# 18 "ResNet50.cpp"
...
#pragma HLS STREAM variable=skip_connection_fifo depth=1024
 ...
 data_t local_in[256];
    data_t local_out[256];


    VITIS_LOOP_36_1: for (int r = 0; r < 56; r++) {
        VITIS_LOOP_37_2: for (int c = 0; c < 56; c++) {


            VITIS_LOOP_40_3: for (int ch = 0; ch < 256; ch++) {
#pragma HLS PIPELINE II=1
 data_t pixel = input_tensor[(ch * 56 * 56) + (r * 56) + c];
                local_in[ch] = pixel;
                skip_connection_fifo.write(pixel);
            }
# 55 "ResNet50.cpp"
            VITIS_LOOP_55_4: for (int ch = 0; ch < 256; ch++) {
                local_out[ch] = local_in[ch] * (data_t)0.5;
            }



            VITIS_LOOP_61_5: for (int ch = 0; ch < 256; ch++) {
#pragma HLS PIPELINE II=1
 data_t f_x = local_out[ch];
                data_t x = skip_connection_fifo.read();

                data_t added = f_x + x;


                output_tensor[(ch * 56 * 56) + (r * 56) + c] = (added > 0) ? added : (data_t)0;
            }
        }
    }
}
