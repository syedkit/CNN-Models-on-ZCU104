...
typedef struct {
    ...
    XAlexnet_layer1_uio_map maps[ MAX_UIO_MAPS ];
} XAlexnet_layer1_uio_info;

/***************** Variable Definitions **************************************/
static XAlexnet_layer1_uio_info uio_info;
...
static int uio_info_read_name(XAlexnet_layer1_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XAlexnet_layer1_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XAlexnet_layer1_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XAlexnet_layer1_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XAlexnet_layer1_Initialize(XAlexnet_layer1 *InstancePtr, const char* InstanceName) {
	XAlexnet_layer1_uio_info *InfoPtr = &uio_info;
