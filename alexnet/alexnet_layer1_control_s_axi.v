...
// ==============================================================
`timescale 1ns/1ps
module alexnet_layer1_control_s_axi
#(parameter
...
localparam
    ...
    ADDR_OUTPUT_FEAT_DATA_0 = 6'h1c,
    ADDR_OUTPUT_FEAT_DATA_1 = 6'h20,
    ADDR_OUTPUT_FEAT_CTRL   = 6'h24,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_OUTPUT_FEAT_DATA_0: begin
                    rdata <= int_output_feat[31:0];
                end
                ADDR_OUTPUT_FEAT_DATA_1: begin
                    rdata <= int_output_feat[63:32];
                end
...
assign output_feat       = int_output_feat;
...
// int_output_feat[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_output_feat[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUTPUT_FEAT_DATA_0)
            int_output_feat[31:0] <= (WDATA[31:0] & wmask) | (int_output_feat[31:0] & ~wmask);
    end
end

// int_output_feat[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_output_feat[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUTPUT_FEAT_DATA_1)
            int_output_feat[63:32] <= (WDATA[31:0] & wmask) | (int_output_feat[63:32] & ~wmask);
    end
end
